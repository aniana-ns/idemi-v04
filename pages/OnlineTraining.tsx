...
import { ArrowLeft, BookOpen, Clock, Award, Search, Building2, GraduationCap, FileText, AlertCircle, Phone, User, Loader2, X, Download, Monitor, Globe, Zap, ChevronUp, Eye, Cpu, Box, Scissors, MessageSquare, Video, Laptop, FileCheck, Info } from 'lucide-react';
  ...
  <div className="mb-6 overflow-hidden rounded-3xl bg-slate-900 text-white shadow-xl relative group reveal-on-scroll">
      ...
      <div className="relative z-20 p-6 md:p-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-500 text-slate-900 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 shadow-xl">
             < Zap size={14} fill="currentColor" /> Virtual Academy
          </div>
          ...
          <p className="text-blue-100/90 text-xs md:text-sm max-w-2xl leading-relaxed mb-6 font-medium">
              IDEMI empowers learners with world-class technical education in cutting-edge fields including <span className="text-amber-400 font-bold">Animation, AR/VR, AI & ML, Robotics, Electronics, IoT, PLC, Electrical Engineering, Fashion Design, and beyond</span>. Our live online classes are led by industry experts, delivering hands-on knowledge and real-world insights directly to you—anytime, anywhere.
          </p>
...
const OnlineTraining: React.FC = () => {
  ...
  const parseCSV = (csv: string) => {
    ...
    return lines.slice(1).map(line => {
      ...
      return {
        ...
        applyLink: values[10] || '',
        imageLink: values[11] || 'https://images.unsplash.com/photo-1588196749597-9ff075ee6b5b?auto=format&fit=crop&q=80'
      };
    });
  };
    ...
    const fetchData = async () => {
      try {
        ...
        const filtered = allData.filter(c => 
          c.type.toLowerCase().includes('online training')
        );
  ...
  }, [isLoading, refreshObserver, searchTerm]);

  const filteredCourses = courses.filter(c => {
    return c.name.toLowerCase().includes(searchTerm.toLowerCase()) || c.description.toLowerCase().includes(searchTerm.toLowerCase());
  });

  return (
    <div className="bg-gray-50 dark:bg-gray-950 min-h-screen transition-colors duration-200">
      <SEO seo={{ title: 'Online Technical Training | IDEMI Mumbai', description: 'Flexible online technical training and virtual classroom programs for students and working professionals.' }} path="/training/online-training" />
      ...
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-[114px] xl:top-[124px] z-30 shadow-sm">
        <div className="container mx-auto px-4 py-3">
             ...
             <h1 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white uppercase tracking-tight">Online Training</h1>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8 flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-3/4">
             ...
             <div className="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 reveal-on-scroll mb-6">
                <div className="max-w-4xl">
                    <p className="text-base text-gray-700 dark:text-gray-200 leading-relaxed mb-8 font-medium">
                        Virtual classroom sessions designed for students and professionals to learn from anywhere. Our online programs maintain the same rigor and quality as our offline courses, featuring live interactions with faculty.
                    </p>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    <div>
                        <h2 className="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tight mb-4 flex items-center gap-2">
                            <Monitor className="text-primary" size={20} /> Online Training
                        </h2>
                        <h3 className="text-[10px] font-black text-secondary dark:text-amber-500 uppercase tracking-widest mb-4">Why Online Training?</h3>
                        <ul className="space-y-2.5">
                            {[
                                { text: "Live Interactive Sessions via Zoom/Google Meet", icon: <Video size={14} /> },
                                { text: "Digital Course Material & Recordings", icon: <FileCheck size={14} /> },
                                { text: "E-Certificate upon completion", icon: <Award size={14} /> },
                                { text: "Flexible Evening Batches for Professionals", icon: <Clock size={14} /> }
                            ].map((item, i) => (
                                <li key={i} className="flex items-center gap-2 text-gray-700 dark:text-gray-300 font-bold text-xs">
                                    <div className="p-1 bg-blue-50 dark:bg-gray-700 rounded text-primary dark:text-blue-400">
                                        {item.icon}
                                    </div>
                                    {item.text}
                                </li>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        <div className="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-700 group hover:border-secondary/30 transition-all">
                            <div className="flex items-center gap-2 mb-1">
                                <Laptop className="text-secondary" size={18} />
                                <h4 className="text-xs font-black text-gray-900 dark:text-white uppercase tracking-tight">Remote Access</h4>
                            </div>
                            <p className="text-[11px] text-gray-500 dark:text-gray-400 font-medium">Access labs via remote desktop for practical software experience.</p>
                        </div>
                        <div className="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-700 group hover:border-secondary/30 transition-all">
                            <div className="flex items-center gap-2 mb-1">
                                <BookOpen className="text-secondary" size={18} />
                                <h4 className="text-xs font-black text-gray-900 dark:text-white uppercase tracking-tight">Recorded Lectures</h4>
                            </div>
                            <p className="text-[11px] text-gray-500 dark:text-gray-400 font-medium">Revisit concepts anytime with permanent access to session recordings.</p>
                        </div>
                    </div>
                </div>
             </div>
             ...
             <div className="bg-white dark:bg-gray-800 p-5 md:p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 reveal-on-scroll">
                ...
                {/* Search - Mode filter removed as per request */}
                <div className="space-y-4 mb-6 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
                    <div className="relative">
                        ...
                        <input 
                            type="text" 
                            placeholder="Filter online courses..." 
                            value={searchTerm}
                ...
                {isLoading ? (
                    <div className="py-16 text-center flex flex-col items-center">
                        ...
                        <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">Accessing Online Registry...</p>
                    </div>
                ) : error ? (
                    ...
                    <div className="space-y-4">
                        {filteredCourses.length > 0 ? filteredCourses.map((course, idx) => {
                                ...
                                <div key={idx} className="bg-white dark:bg-gray-700/10 rounded-2xl p-0 border border-gray-100 dark:border-gray-700 hover:shadow-lg transition-all group relative overflow-hidden flex flex-col">
                                    <div className="flex flex-col md:flex-row">
                                        ...
                                        <div className="flex-grow p-4 md:p-5 flex flex-col">
                                            ...
                                            <h3 className="text-base font-bold text-gray-900 dark:text-white mb-2 group-hover:text-primary transition-colors leading-tight">{course.name}</h3>
                                        ...
                                        <div className="p-4 animate-fade-in bg-slate-50 dark:bg-slate-900/50 border-t border-gray-100 dark:border-gray-700">
                                            <div className="bg-white dark:bg-gray-900 rounded-xl border-2 border-secondary/20 overflow-hidden shadow-xl">
                                                <div className="bg-secondary p-2 flex justify-between items-center text-white">
                                                    <span className="text-[10px] font-black uppercase tracking-widest flex items-center gap-1"><FileText size={12} /> Syllabus</span>
                            ...
                            <div className="py-12 text-center bg-gray-50 dark:bg-gray-900 rounded-3xl border-2 border-dashed border-gray-300">
                                <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">No matching online programs found</p>
                            </div>
                        )}
                    </div>
                )}
