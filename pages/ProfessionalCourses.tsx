...
const CourseDomainBanner = () => (
  <div className="mb-6 overflow-hidden rounded-3xl bg-slate-900 text-white shadow-xl relative group reveal-on-scroll">
      ...
      <img
          ...
          alt="Professional Technical Workshops"
      />
      <div className="relative z-20 p-6 md:p-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-500 text-slate-900 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 shadow-xl">
             <Zap size={14} fill="currentColor" /> Industry Certifications
          </div>
          ...
          <p className="text-blue-100/90 text-xs md:text-sm max-w-2xl leading-relaxed mb-6 font-medium">
              IDEMI empowers learners with world-class technical education in cutting-edge fields including <span className="text-amber-400 font-bold">Animation, AR/VR, AI & ML, Robotics, Electronics, IoT, PLC, Electrical Engineering, Fashion Design, and beyond</span>. Our industry-focused professional workshops are led by experts, delivering specialized hands-on knowledge and real-world insights—anytime, anywhere.
          </p>
...
const ProfessionalCourses: React.FC = () => {
  ...
  const parseCSV = (csv: string) => {
    ...
    return lines.slice(1).map(line => {
      ...
      return {
        ...
        applyLink: values[10] || '',
        imageLink: values[11] || 'https://images.unsplash.com/photo-1558346490-a72e53ae2d4f?auto=format&fit=crop&q=80'
      };
    });
  };
  ...
  const ModeBadge = ({ mode }: { mode: string }) => {
    ...
    if (m.includes('online')) {
      return <span className="text-[9px] font-black uppercase tracking-widest text-blue-700 bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded border border-blue-100 dark:border-blue-800 flex items-center gap-1"><Monitor size={10} /> {mode}</span>;
    }
    if (m.includes('hybrid')) {
      return <span className="text-[9px] font-black uppercase tracking-widest text-amber-700 bg-amber-50 dark:bg-amber-900/20 px-2 py-0.5 rounded border border-amber-100 dark:border-amber-800 flex items-center gap-1"><Globe size={10} /> {mode}</span>;
    }
    return <span className="text-[9px] font-black uppercase tracking-widest text-emerald-700 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-0.5 rounded border border-emerald-100 dark:border-emerald-800 flex items-center gap-1"><Building2 size={10} /> {mode}</span>;
  };
    ...
    const fetchData = async () => {
      try {
        ...
        const filtered = allData.filter(c => 
          c.type.toLowerCase().includes('professional workshops')
        );
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-950 min-h-screen transition-colors duration-200">
      <SEO seo={{ title: 'Professional Technical Workshops | IDEMI Mumbai', description: 'Specialized professional workshops and certification programs for engineering and industry professionals.' }} path="/training/professional-courses" />
      ...
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-[114px] xl:top-[124px] z-30 shadow-sm">
        <div className="container mx-auto px-4 py-3">
             ...
             <h1 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white uppercase tracking-tight">Professional Certifications</h1>
        </div>
      </div>
             ...
             <div className="bg-white dark:bg-gray-800 p-5 md:p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 reveal-on-scroll">
                ...
                <div className="space-y-4 mb-6 bg-gray-50 dark:bg-gray-900/50 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
                    <div className="relative mb-4">
                        ...
                        <input 
                            type="text" 
                            placeholder="Filter professional programs..." 
                            value={searchTerm}
                ...
                {isLoading ? (
                    <div className="py-16 text-center flex flex-col items-center">
                        ...
                        <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">Syncing Certification Data...</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {filteredCourses.length > 0 ? filteredCourses.map((course, idx) => {
                                    ...
                                    <div className="flex flex-col md:flex-row">
                                        ...
                                        <div className="flex-grow p-4 md:p-5 flex flex-col">
                                            <div className="flex items-center gap-2 mb-2">
                                                <span className="px-2 py-0.5 bg-primary text-white text-[9px] font-black uppercase rounded shadow-sm">NSQF {course.nsqf || 'Expert'}</span>
                            ...
                            <div className="py-12 text-center bg-gray-50 dark:bg-gray-900 rounded-3xl border-2 border-dashed border-gray-300">
                                <p className="text-gray-400 font-bold uppercase tracking-widest text-[10px]">No matching professional programs found</p>
                            </div>
                        )}
                    </div>
                )}
